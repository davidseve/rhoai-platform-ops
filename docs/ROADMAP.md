...
- [x] **Gateway production hardening** -- Authorino: 2 replicas + resource limits (250m-1 CPU, 512Mi-1Gi memory) via PostSync Job. Limitador: 2 replicas + resource limits (100m-500m CPU, 128-256Mi memory) via Limitador CR. PDBs for both (minAvailable: 1). Two new alerts: `MaaSAuthTimeoutRateHigh` (auth errors >1%) and `MaaSAuthorinoCPUSaturation` (CPU >80%). E2E tests for replicas, resources, PDBs. Documentation in GATEWAY-AND-ROUTE.md.
