...
- [x] **Gateway production hardening** -- Limitador: 2 replicas + resource limits (100m-500m CPU, 128-256Mi memory) via Limitador CR. PDBs for Authorino and Limitador (minAvailable: 1). Two new alerts: `MaaSAuthTimeoutRateHigh` (auth errors >1%) and `MaaSAuthorinoCPUSaturation` (CPU >80%). E2E tests for Limitador replicas and PDBs. Documentation in GATEWAY-AND-ROUTE.md. **Pending**: Authorino replicas and resource limits — CRD (`v1beta2`) does not expose `spec.resources`; re-evaluate when Authorino operator adds support.
