...
- **TelemetryPolicy CEL incompatibility (deferred)**: `responseBodyJSON("/model")` and `auth.identity.selected_subscription` are Kuadrant WASM expressions, NOT valid Authorino CEL. Tenant CR with `telemetry.enabled: true` auto-creates a TelemetryPolicy with these expressions. Authorino logs `failed to parse CEL expression` errors — **non-fatal** (metric label evaluation only, not auth decisions). **Impact of leaving disabled**: Limitador metrics (`kuadrant_allowed`, `kuadrant_denied`) lack `model`, `subscription`, `organization_id`, and `cost_center` labels. Per-model and per-subscription usage attribution in dashboards/alerts is not possible — only aggregate gateway-level metrics. FinOps/showback views will not work. **Workaround applied**: Tenant patched to `telemetry.enabled: false`, TelemetryPolicy deleted. **Re-evaluate in RHOAI 3.5+** when Kuadrant may separate WASM and Authorino CEL expression evaluation.
...
- **Gateway→Authorino listener TLS (not feasible, 2026-05-18)**: The upstream `setup-authorino-tls.sh` enables `listener.tls.enabled: true` on the Authorino CR. This causes 500 errors because the Kuadrant EnvoyFilter (`kuadrant-auth-maas-default-gateway`) creates a custom cluster `kuadrant-auth-service` without `transport_socket` — Envoy sends plaintext gRPC but Authorino expects TLS. Attempted fix: supplementary EnvoyFilter with MERGE to add `transport_socket`. Result: MERGE does not apply to clusters created via ADD by other EnvoyFilters. The Gateway pod already has service-ca at `/var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt`, so the cert is available — the problem is purely the Kuadrant EnvoyFilter. **Current approach**: keep `listener.tls.enabled: false`, use PostSync Job for outbound CA trust only (Authorino→maas-api). Internal Gateway→Authorino traffic stays plaintext (intra-cluster, protected by NetworkPolicy). **Re-evaluate** when CONNLINK-528 is resolved (Kuadrant supports `transport_socket` in generated EnvoyFilters).
